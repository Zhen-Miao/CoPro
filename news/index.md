...
- Multi-slide `@pcaResults` stores per-slide *views* of the global PC
  ...
  per cell type instead of once per (slide, cell type). Slices are still
  materialized when `center_per_slide = TRUE`, where re-centering makes
  them genuinely different data, and objects saved with materialized
  slices continue to work. The slot’s shape – one entry per slide, keyed
  by slide name – is unchanged.
