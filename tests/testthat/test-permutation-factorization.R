...
test_that("the permutation worker does not capture the kernels", {
  ...
  # `permu_objective` carries the criterion the null must be built with, and
  # `permu_grams` the Gram matrices a draw is allowed to inherit rather than
  # recompute (a `NULL` per cell type where it is not). Both are nPCA x nPCA
  # per cell type, so they do not change the character of what is shipped to a
  # worker; the size assertions below still apply.
  expect_setequal(
    ls(captured),
    c("PCmats", "plan", "cts", "nCC", "sdev2_list", "maxIter", "tol",
      "permu_objective", "permu_grams", "sumcor")
  )
