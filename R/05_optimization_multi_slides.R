...
# ============================================================================
# Helper Functions (that are not available from 04_optimization_function_refactored.R)
# ============================================================================

#' Helper function to safely get kernel matrix K_ij or transpose of K_ji
#' 
#' This function retrieves kernel matrices from a nested list structure.
#' If \code{K_list[[ct_i]][[ct_j]]} is NULL, it tries to get \code{K_list[[ct_j]][[ct_i]]} and transpose it.
#' 
...
#' Check convergence by computing maximum difference between weight vectors
#' @param w_list_new New weight list
#' @param w_list_old Old weight list
#' @param cell_types Cell type names
#' @return Maximum difference across all cell types
#' @noRd
check_convergence <- function(w_list_new, w_list_old, cell_types) {
  ...
  for (ct in cell_types) {
    ...
    if (!identical(dim(w_new), dim(w_old))) {
      stop(paste("Dimension mismatch for cell type:", ct))
    }
    
    diff_val <- max(abs(w_new - w_old))
...
#' @param Y_resi Named list of residual matrices with structure \code{Y_resi[[celltype1]][[celltype2]]}
#' @param n_features Number of features
...
# Main function aliases - original names for backward compatibility
optimize_bilinear_slides <- optimize_bilinear_multi_slides
optimize_bilinear_n_slides <- optimize_bilinear_n_multi_slides

# Aliases for single cell type functions
optimize_bilinear_multi_slides_one <- function(...) {
  optimize_bilinear_multi_slides(...)
}

optimize_bilinear_multi_slides_one_iter <- function(..., direct_solve = FALSE) {
  optimize_bilinear_multi_slides(..., direct_solve = direct_solve)
}

optimize_bilinear_n_multi_slides_one <- function(...) {
  optimize_bilinear_n_multi_slides(...)
}

# Single cell type original function names
optimize_bilinear_slides_one <- function(...) {
  optimize_bilinear_multi_slides(...)
}

optimize_bilinear_slides_one_iter <- function(..., direct_solve = FALSE) {
  optimize_bilinear_multi_slides(..., direct_solve = direct_solve)
}

optimize_bilinear_n_slides_one <- function(...) {
  optimize_bilinear_n_multi_slides(...)
}

# The original bilinear_w_from_Y_resi_multi_slides_one is now handled by
# the general bilinear_w_from_Y_resi function

# The original bilinear_w_from_Y_resi_slides is also handled by
# the general bilinear_w_from_Y_resi function

# Additional backward compatibility aliases -- to be removed in the future
optimize_bilinear_multi_n_slides <- optimize_bilinear_n_multi_slides