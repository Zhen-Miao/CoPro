...
.compute_g12_inhom <- function(X, type_i, type_j, r_vec, sigma_intensity, edge_correction = "translation") {
  ...
  lambdaI <- density.ppp(Xi, sigma = sigma_intensity, at = "pixels", edge = TRUE)
  lambdaJ <- density.ppp(Xj, sigma = sigma_intensity, at = "pixels", edge = TRUE)

  # pcfcross.inhom uses intensity pixel images for both types
  g <- pcfcross.inhom(X, i = type_i, j = type_j,
...
coloc_score <- function(
  ...
  win <- owin(xrange = window_range$xrange * s, yrange = window_range$yrange * s)

  # Build multitype pattern
  X <- superimpose(
    A = ppp(Au$x, Au$y, window = win, check = FALSE),
    B = ppp(Bu$x, Bu$y, window = win, check = FALSE),
    W = win, check = FALSE
  )
  marks(X) <- factor(marks(X), levels = c("A","B"))

  nA <- sum(marks(X) == "A"); nB <- sum(marks(X) == "B")
  ...
  for (k in seq_len(nsim)) {
    X_sim <- rlabel(X)  # keeps locations, shuffles marks
    # Ensure mark levels preserved
    marks(X_sim) <- factor(marks(X_sim), levels = c("A","B"))
...
nn_cross_fraction <- function(A, B, window_range, pixel_size_um = 1) {
  ...
  dA_B <- nncross(X[X$marks == "A"], X[X$marks == "B"], what = "dist")$dist
  dA_A <- nndist(X[X$marks == "A"], what = "dist")
  dB_A <- nncross(X[X$marks == "B"], X[X$marks == "A"], what = "dist")$dist
  dB_B <- nndist(X[X$marks == "B"], what = "dist")
