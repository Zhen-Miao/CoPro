...
# ============================================================
# 1c. Colon Day 0 (multi-slide) -- gene-space CCA joint analysis
#     D0 = healthy colon, 10 slides across 4 regions.
#     We select 3 slides from different regions.
#     Preprocessing: gene filter >= 0.8%, cell filter >= 20 genes,
#                    98th percentile cap, coordinates /10.
# ============================================================
message("\n=== Preparing Colon D0 (multi-slide) ===")

d0_ra <- readRDS(file.path(DATA_ROOT,
  "ra_filtered_healthy_colon_all_slides.rds"))
d0_meta <- readRDS(file.path(DATA_ROOT,
  "meta_filtered_healthy_colon_all_slides.rds"))
d0_meta <- as.data.frame(d0_meta)

# Gene filter: >= 0.8% cells with expression > 0
d0_bin <- d0_ra
...
# Cap at 98th percentile (matching gene-space CCA default clip)
quant_98 <- quantile(d0_ra@x, probs = 0.98)
d0_ra@x[d0_ra@x > quant_98] <- quant_98
message("  98th percentile cap: ", quant_98)

# Select 3 slides from different regions
selected_slides <- c("062921_D0_m3a_1_slice_1",   # Region 3
                      "082421_D0_m6_1_slice_1",    # Region 2
                      "082421_D0_m7_1_slice_1")    # Region 1
...
saveRDS(d0_data, "data-raw/vignette_data/copro_colon_d0_multi.rds", compress = "xz")
message("  Saved: ", round(file.size("data-raw/vignette_data/copro_colon_d0_multi.rds") / 1e6, 1), " MB")
