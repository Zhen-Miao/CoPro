...
`z = (T - mean_null) / sd_null`. Both the location and the scale come from the
null itself, which is what makes the values comparable across the grid; within a
single cell type the location term is not decorative, because the convention that
a cell is not its own neighbour gives the null a sigma-dependent mean of its own.
Because one pass of draws evaluates every bandwidth, the same draws also give the
null distribution of `max z`, so the reported p-value already accounts for having
scanned the grid.
